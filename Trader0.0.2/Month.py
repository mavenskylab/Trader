...
SAVE_PATH = "saved_model/month_model"
...
if CLASSIFY:
    ...
    for i, TICKER in enumerate(TICKER_LIST):
        ...
        df = df.resample("BMS").agg({"open": "first", "high": "max", "low": "min", "close": "last"})
        df = classify_df(df)
        data = data + preprocess_df(df)

    print()
    save(data, "month_data")
...
sequential_data = read("month_data")
...
checkpoint_dir = "./checkpoints/month_checkpoints"
checkpoint_prefix = os.path.join(checkpoint_dir, "ckpt_{epoch}")

callbacks = [
    tf.keras.callbacks.TensorBoard(log_dir="./logs/month_logs"),
