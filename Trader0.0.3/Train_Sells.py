...
SAVE_PATH = "saved_model/sell_model"
...
def balance_data(sequential_data):
    ...
    lower = min(len(sells), len(holds))

    sells = sells[:lower]
    holds = holds[:lower]

    sequential_data = sells + holds
    np.random.shuffle(sequential_data)
...
def split_data(sequential_data):
    ...
    print(f"Sells: {y.count(1)}, Holds: {y.count(0)}, Sell%: {y.count(1)/len(y)}")
...
sequential_data = read("sell_data")
